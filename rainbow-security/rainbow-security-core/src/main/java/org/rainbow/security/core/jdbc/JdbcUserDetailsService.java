...
public class JdbcUserDetailsService extends JdbcDaoImpl {
	...
	public static final String DEF_AUTHORITIES_BY_USERNAME_QUERY = "SELECT u.USER_NAME, r.NAME FROM USERS u INNER JOIN ROLES r ON "
			+ "u.ROLE_ID=r.ID INNER JOIN APPLICATIONS a ON u.APPLICATION_ID=a.ID " + "WHERE u.USER_NAME=? AND a.NAME=?";
	...
	private final String passwordExpiredQuery = "SELECT m.LAST_PWD_CHANGE_DATE AS LAST_PWD_CHANGE_DATE,m.CREATION_DATE AS CREATION_DATE,pp.MAXIMUM_PASSWORD_AGE AS MAXIMUM_PASSWORD_AGE, pp.MINIMUM_PASSWORD_AGE "
	...
	public JdbcUserDetailsService(String applicationName) {
		super();
	...
	protected List<GrantedAuthority> loadUserAuthorities(String username) {
		return getJdbcTemplate().query(super.getAuthoritiesByUsernameQuery(),
				new String[] { username, applicationName }, new RowMapper<GrantedAuthority>() {
					@Override
	...
	protected List<GrantedAuthority> loadGroupAuthorities(String username) {
		return getJdbcTemplate().query(this.groupAuthoritiesByUsernameQuery, new String[] { username, applicationName },
	...
	private Boolean passwordExpired(String username) {
		return getJdbcTemplate().queryForObject(passwordExpiredQuery, new String[] { username, applicationName },
				new RowMapper<Boolean>() {
					@Override
					public Boolean mapRow(ResultSet rs, int rowNum) throws SQLException {
						Date lastPasswordChangeDate = rs.getDate("LAST_PWD_CHANGE_DATE");
						Date creationDate = rs.getDate("CREATION_DATE");
						int maximumPasswordAge = rs.getInt("MAXIMUM_PASSWORD_AGE");
						int minimumPasswordAge = rs.getInt("MINIMUM_PASSWORD_AGE");
