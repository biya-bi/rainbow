 ...
 * include ambiguous characters, such as I, l, and 1. The generated password
 * will be made of 7-bit ASCII symbols. Every four characters will include one
 * lower case character, one upper case character, one number, and one special
 * symbol (such as '%') in a random order. The password will always start with
 * an alpha-numeric character; it will not start with a special symbol (we do
 * this because some back-end systems do not like certain special characters in
 * the first position).
 */
public class PasswordGenerator {
    ...
    public PasswordGenerator(int minLength, int maxLength, int minSpecialChars, int minLowerCase, int minUpperCase, int minNumeric) {
    ...
    public String generate() {
        ...
        for (int i = 0; i < password.length; i++) {
            ...
            if (lastLeftGroupsOrderIdx == 0) {
                ...
                nextLeftGroupsOrderIdx = random.nextInt(lastLeftGroupsOrderIdx + 1); // Biya-Bi incremented this by one.
            }
    ...
    private char[] setMinimumChars(String characterSource, char[] password, int minNonalphanumericChars, Random random) {
        ...
        for (int i = 0; i < password.length; i++) {
            // If there is no charecter at that index,add the index to the empty indices list.
            if (password[i] == '\0') {
