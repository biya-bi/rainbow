...
import org.rainbow.persistence.dao.ListValuedFilter;
import org.rainbow.persistence.dao.RangeValuedFilter;
...
public abstract class Dao<T> implements IDao<T> {
	...
	@Override
	public void create(Collection<T> entities) throws Exception {
	...
	public void delete(Collection<T> entities) throws Exception {
		...
		Map<Object, T> entitiesByIds = new HashMap<>();
		...
		List<T> persistentEntities = query.getResultList();

		for (T persistentEntity : persistentEntities) {
	...
	protected void onDelete(T entity, T persistentEntity) {
	}

	protected void remove(T entity) throws NonexistentEntityException {
		T persistentEntity = getPersistent(entity);
	...
	public T getPersistent(T entity) throws NonexistentEntityException {
		try {
			...
			PersistenceUnitUtil util = em.getEntityManagerFactory().getPersistenceUnitUtil();
			Object id = util.getIdentifier(entity);
			T persistentEntity = em.getReference(entityClass, id);
	...
	@Override
	public List<T> find(SearchOptions options) {
	...
	private <U> Expression<Boolean> getExpression(List<Filter<?>> filters, CriteriaBuilder cb, Root<T> rt) {
	...
	private <U> Path getPath(Filter<U> filter, Root<T> rt) {
